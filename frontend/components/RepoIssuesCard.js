...
import { CircleDot, CheckCircle2, AlertCircle, Loader2, Tag, MessageSquare } from 'lucide-react';

export default function RepoIssuesCard({ issues, error, loading, selectedIssueNumber, onSelectIssue }) {
    ...
    <div className="border border-terminal-border rounded bg-terminal-surface flex flex-col h-full overflow-hidden terminal-hover">
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-terminal-border flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-terminal-text text-xs">repo issue backlog</span>
        </div>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-3 py-2 space-y-0">
        {loading ? (
          ...
          filtered.map((issue) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left py-2.5 px-1 rounded border transition ${
                  isSelected
                    ? 'border-terminal-text bg-terminal-dim'
              ...
              >
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-terminal-text text-xs truncate">{issue.title}</p>
                    <div className="flex items-center gap-3 mt-1 text-terminal-muted text-xs">
          ...
          <p className="text-terminal-muted text-xs pt-3">{`// no ${filter} issues returned`}</p>
        )}
      </div>
    </div>
  );
